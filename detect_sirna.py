...
class SeqReader:
    ...
    #     """ AU rich points"""
    #     if mrna[14:18].count('A') + mrna[14:18].count('U') + mrna[14:18] > 0:
    ...
    def check_uitei_design(self, sense_strand: Seq) -> tuple:
        ...
        if (sense_strand[18] != "A") or (sense_strand[18] != "U") or (sense_strand[18] != "T"):
            uitei_rules["A/U not in position 19"] =CGREEN+ sense_strand[18]+CEND
            points += 1
        else:
            uitei_rules["A/U not in position 19"] = CRED+ sense_strand[18] +CEND
        """Rule 2"""
        if (sense_strand[0] == "G") or (sense_strand[0] == "C"):
            uitei_rules["G/C on position 1"] =CGREEN+ sense_strand[0]+CEND
            points += 1
        else:
            uitei_rules["G/C on position 1"] = CRED + sense_strand[0] + CEND
        """Rule 3"""
        if sense_strand[9:16].count('A') + sense_strand[9:16].count('U') + sense_strand[9:16].count('T') > 4:
            uitei_rules['A/U rich within 10-17'] =CGREEN + str(sense_strand[9:16].count('A') +\
                                                   sense_strand[9:16].count('U') + \
                                                   sense_strand[9:16].count('T')) + CEND
            points += 1
        else:
            uitei_rules['A/U rich within 10-17'] =CRED+ sense_strand[9:16].count('A') +\
                                                   sense_strand[9:16].count('U') + \
                                                   sense_strand[9:16].count('T') + CEND
        """Rule 4"""
        ...
        for element in gc_stretch_check:
            if GC(element) == 0:
                uitei_rules["GC stretch < 10"] = CRED+ "Present"+CEND
                break
            else:
                stretch_count += 1
        if len(gc_stretch_check) == stretch_count:
            uitei_rules["GC stretch < 10"] =CGREEN+ "Absent"+CEND
            points += 1
        """Rule 5"""
        ...
        if diff > 5:
            uitei_rules["Asymmetrical 5'/3' Tm"] =CGREEN+ f"Difference: {diff}C, OK" +CEND
            points += 1
        else:
            uitei_rules["Asymmetrical 5'/3' Tm"] =CRED+ f"Difference: {diff}C, BAD"+CEND
        """Rule 6"""
        if tm_at_5-tm_at_3 >= 10:
            uitei_rules["Unstable 3' low Tm"] =CGREEN+ f"Passed, Tm at 3' END = {tm_at_3}C, dT > 10"+CEND
            points += 1
        else:
            uitei_rules["Unstable 3' low Tm"] = CRED+ f"Failed, Tm = {tm_at_3}, dT < 10"+CEND
        """Rule 7"""
        total_mt = round(mt.Tm_NN(seq = sense_strand, c_seq=sense_strand.complement()),1)
        if total_mt <= 21.5:
            uitei_rules["Total Tm"]=f"{CGREEN+total_mt}C{CEND}"
            points += 1
        else:
            uitei_rules["Total Tm"] = f"{CRED+str(total_mt)}C, higher than 21.5C, FAIL {CEND}"
    ...
    def check_reynolds_algorithm(self, mrna: Seq) -> tuple:
        ...
        if (GC(mrna) < 52) and (GC(mrna) > 30):
            reynolds_points += 1
            reynolds_rules["GC content"] = CGREEN + str(int(GC(mrna))) + CEND
        else:
            reynolds_rules["GC content"] = CRED + str(int(GC(mrna))) + CEND
        """Rule 2"""
        if mrna[14:18].count('A') + mrna[14:18].count('U') + mrna[14:18].count('T') > 0:
            reynolds_rules["At least 4 A/U on bp 15-19"] = f"{CGREEN}PASSED{CEND}," \
                                                                f" {mrna[14:18].count('A') + mrna[14:18].count('U') + + mrna[14:18].count('T')}"
            reynolds_points += mrna[14:18].count('A') + mrna[14:18].count('U')
        else:
            reynolds_rules['At least 4 A/U on bp 15-19'] = f"{CRED}FAILED{CEND}, 0"
        """Rule 3"""
        check_reps = 0
        for index, nucleotide in enumerate(mrna):
            try:
        ...
        if check_reps == 0:
            reynolds_rules["internal repeats"] = CGREEN+'0'+CEND
            reynolds_points += 1
        else:
            reynolds_rules["internal repeats"] = CRED + str(check_reps) + CEND

        """Rule 4"""
        if mrna[2] == 'A':
            reynolds_points += 1
            reynolds_rules["A at 3nt"] = CGREEN+"A"+CEND
        else:
            reynolds_rules["A at 3nt"] = CRED+mrna[2]+CEND

        """Rule 5"""
        if mrna[18] == 'A':
            reynolds_points += 1
            reynolds_rules["A at 19nt"] = CGREEN+"A"+CEND
        else:
            reynolds_rules["A at 19nt"] = CRED+mrna[18]+CEND
        """Rule 6"""
        if mrna[9] == 'U' or mrna[9] == 'T':
            reynolds_points += 1
            reynolds_rules["U at 10nt"] = CGREEN+mrna[9]+CEND
        else:
            reynolds_rules["U at 10nt"] = CRED+mrna[9]+CEND
        """Rule 7"""
        if (mrna[18] == 'G') or (mrna[18] == 'C'):
            reynolds_points -= 1
            reynolds_rules["G/C not at 19nt"] = CRED+mrna[18]+CEND
        else:
            reynolds_rules["G/C not at 19nt"] = CGREEN+mrna[18]+CEND
        """Summary"""
        reynolds_rules["Result"] = reynolds_points
        if reynolds_points >= 6:
            return True, reynolds_rules
        else:
            return False, reynolds_rules
...
if __name__ == '__main__':
    ...
    for mrna in subseqlist:
        ...
        if testcase.check_uitei_design(mrna)[0]:
            if testcase.check_reynolds_algorithm(mrna)[0]:
                ...
                #     print(f'{key}: {value}')
                print("\n")
                print(f"{CGREEN_BACKGROUND}Ui-tei algorithm:{CEND} \n")
                for key, value in testcase.check_uitei_design(mrna)[1].items():
                    print(f'{key}: {value}')

                print(f"{CBLUE}Reynolds algorithm{CEND}\n")
                for key, value in testcase.check_reynolds_algorithm(mrna)[1].items():
                    print(f'{key}: {value}')
                print("\n")
